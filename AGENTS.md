...
- **合入门禁：** 一切变更走 feature 分支 + PR，禁止直推 main。已绿、范围清楚、非红线候选，在作者为可信同仓 `NicholasHan1226`、非 fork、非 draft、base 为 `main`、精确当前 head 的 TradingDatas CI 成功时，可由该作者的主助手直接 `gh pr merge`，或由 automerge 在同样条件下自动合并。Datas PM `pm-merge` 标签是可选加速或事后审计，缺席不得阻断。`controller-accepted`、`AUTODEV_RETURN_V1` 评论、`automerge-m0` 与 `change_class=M0` 文书已退役。自动合并必须等待 GitHub 的真实 merged SHA，再调度该 SHA 的 main CI；若本次改动含 `static/**` 或 `public-web/**`，同一流程显式调度该 SHA 的 Cloudflare 发布，因为 Actions 身份合并不可靠地重新触发 push workflow。各层 CI、服务器、有效 release、runtime、receipt 和消费者 readback 独立结论：下游未完成只阻断对应发布/运行声明。自动化绝不替 owner 改写候选分支。修改 `.github/workflows/**` 的 PR 不走 automerge，以免门禁自授权；CI 通过后作者 `NicholasHan1226` 可直接经 GitHub 合并，不要求 Datas PM。CI 在 push main 后补跑不构成豁免：直推意味着坏代码可能已在 main 上并被服务器 5 分钟内拉取。本地测试通过不能替代 CI 门禁。GZ/immutable runtime 不会因 GitHub 合并而自动部署。
...
- 已授权的 immutable 发布统一使用 `tools/safe_release.py`；具体证据格式、准备、恢复与回滚以 `docs/OPERATIONS.md` 为唯一入口。会话必须在同一组 release 目录锁内完成验证、排空、切换、读回与状态恢复，记录 actor、task ID 和独立审计文件；需要精确 main CI 及同目标的新进程/同运行面并发 catalog <15s 实测证据。已处于目标版本或保守白名单内的纯文档差异不重复切换。低层 manifest switch 保留兼容与回滚用途，不作为常规发布入口；此流程防止遵守目录锁的发布并行，不能宣称限制 root 的任意手工改链或伪造证据。
