...
export function DocumentationPage({ locale, slug, documentation, onNavigate }) {
  ...
  return <section className="documentation" aria-label="Doc">
    <aside className="doc-sidebar"><a className="doc-wordmark" href="/docs" onClick={link("/docs")}>Doc</a><div className="doc-desktop-directory">{sidebar}</div><details className="doc-mobile-directory" key={slug || "home"}><summary>{zh ? "浏览文档目录" : "Browse documentation"}</summary>{sidebar}</details></aside>
    <div className="doc-main" key={`${locale}-${slug}`}>
      {!slug ? <>
        ...
        <header className="doc-header doc-reader-header"><nav aria-label={zh ? "当前位置" : "Breadcrumb"}><a href="/docs" onClick={link("/docs")}>Doc</a><span aria-hidden="true">/</span><span>{category?.label}</span></nav><h1>{guide.title}</h1><p>{guide.description}</p></header>
      ...
      </> : <header className="doc-header"><h1>{zh ? "没有找到这篇文档" : "Guide not found"}</h1><p>{zh ? "请从左侧目录选择文档，或返回文档首页。" : "Choose a guide from the directory, or return to the overview."}</p><a className="text-link" href="/docs" onClick={link("/docs")}>{zh ? "返回 Doc" : "Back to Doc"}<ArrowRight /></a></header>}
    </div>
  </section>;
}
