  ...
  return (
    <section className="data-source-page">
      ...
      <section className="source-evidence-section">
        ...
        <div className="source-preflight-groups">{pausedContractPreflightSnapshot.groups.map((group) => <article key={group.id}><header><span>{preflightLabel(group.id)}</span><strong>{group.interfaces.length}</strong></header>{group.id === "requires_activation_window_contract" && <p>{locale === "zh" ? "可验证上游，采集窗口合同仍待补齐；不代表已经启用采集。" : "Upstream validation is possible, but the collection-window contract still needs completion; collection is not activated."}</p>}<div>{group.interfaces.map((item) => <div key={item.apiName}><code>{item.apiName}</code><small>{item.datasetId}</small></div>)}</div></article>)}</div>
