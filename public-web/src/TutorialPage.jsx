...
import { ArrowRight, ArrowSquareOut, BookmarkSimple, Copy, Play } from "@phosphor-icons/react";
...
export default function TutorialPage({ id, locale, onNavigate, saved, onToggleBookmark }) {
  ...
  return <article className="object-detail-page research-record tutorial-page">
    ...
    </div><aside className="research-reader-aside" aria-label={zh ? "教程导航" : "Tutorial navigation"}><nav aria-label={zh ? "本页目录" : "On this page"}><h2>{zh ? "本页内容" : "On this page"}</h2>{[["tutorial-inputs", "准备输入", "Inputs"], ["tutorial-step-1", "操作步骤", "Steps"], ["tutorial-example", "运行示例", "Example"], ["tutorial-pitfalls", "常见错误", "Pitfalls"], ["tutorial-real-data", "真实数据", "Real data"]].map(([id, zhLabel, enLabel]) => <a key={id} href={`#${id}`}>{zh ? zhLabel : enLabel}</a>)}</nav><section><h2>{zh ? "相关研究" : "Related research"}</h2><div className="research-next-list">{related.filter(Boolean).map((paper) => <a key={paper.id} href={`/research/${paper.id}`} onClick={(event) => onNavigate(event, `/research/${paper.id}`)}>{researchTitle(paper, locale)}<ArrowRight /></a>)}</div></section></aside></div>
  </article>;
}
