...
export const pythonFunctionNames = { "adjusted-price-series": "adjust_prices", "pit-fundamentals-panel": "select_as_of", "company-event-timeline": "align_events", "minute-bar-gaps": "audit_bar_grid", "document-version-ledger": "preserve_document_versions", "crypto-observation-alignment": "align_spot_and_open_interest" };
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const count = Object.keys(preparationTutorials).length;
  console.log(`Prepared ${count} synthetic input files, ${count} JavaScript examples and ${count * 2} localized Python notebooks.`);
}
