...
async function upstream(ctx,path,key,init={},bodyLimit=512*1024) {
  ...
  const response=await ctx.fetchImpl(new URL(path,base).toString(),{...init,headers,redirect:'manual',signal:AbortSignal.timeout(path==='/v1/catalog' && bodyLimit===ACCOUNT_CATALOG_MAX_BYTES?30000:8000)});
