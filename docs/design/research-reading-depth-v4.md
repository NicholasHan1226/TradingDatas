...
Canonical/share URLs use the trailing-slash directory form generated by this
build, matching Cloudflare Assets' existing `auto-trailing-slash` default. Existing
slashless article links and bookmark IDs remain valid. Vite preview serves the
root SPA shell for some slashless URLs, so verify static metadata on directory
URLs and verify redirect behavior separately in local Wrangler or production.
