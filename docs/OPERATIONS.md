...
按需采集验收须区分默认查询与 `include_receipt_proofs=true`：后者要求同页属于同一
采集序列。registry 合法允许的空日期窗口可以附带证明；有必需日期的合同仍须提供并
验证窗口。对历史行跨序列的数据集，可用单行页或明确合同过滤验证逐行证明，不能为使
整页通过而取消序列约束。catalog success/物理行数不能替代这项查询验收。
...
`cn.dataset.fina_mainbz` 已是 `activation_state=active`，但 on_demand 加上
run-clock `start_date`/`end_date` 与 `type=P` 从未产生 receipt（#349）。
QuickSync 无 `ann_date`。当前合同改为与 `pledge_stat` / forecast 相同的
ts_code-only `entity_fanout`（`batch_size=1`、`max_batches_per_run=1`），
...
次日 08:30 更新上一日数据：`daily_reference` 00:30 与 `postclose_daily`
16:30 都会请求当日尚未发布的分区。当前合同改为通用
`prior_open_morning`：开市日 08:30 之后请求上一开市日 `trade_date`。
empty 仍是 empty，不是 success；历史 trusted-empty 分区保持 empty。
...
news.flash、rt_etf_min*）保持暂停。`etf_mins` / `ft_mins` 已补既有 `max_batches_per_run=1`；2026-09-06 仓外 HTTPS 探测（`evidence/20260906-etf-ft-mins-probe/`，无 facts 写入）里 `etf_mins` 对已落库 `510300.SH` 在 `2026-09-04` 全日窗 nonempty success 241 后写入 `active_evidence`，`ft_mins` 对已落库 `A2609.DCE` 为 `permission_denied` 保持暂停，本切片不采集、不切 GZ `current`。正式采集第一批 lexical `158000.SZ` 为 vendor `provider_error` 20002（非 ETF）；该码官方 `list_status=P`，探测成功码与官方示例 `159001.SZ` 为 `L`，故 `etf_mins` fanout 只加官方 `source_equals.list_status=L`，不发明 `ts_code`。2026-09-06 仓外 HTTPS 窗口探测（`evidence/20260906-bak-fund-window-probe/`，无 facts 写入）对上一完整交易日 `20260904`：`bak_daily` 5566 与 `fund_manager` 3 为 nonempty <10000，已写入 `active_evidence`；`fund_adj` 正好 2000，记整千边界 `row_limit_observation` / `reject_at_limit=true`，保持暂停，不抬高 10000。未 restage `etf_mins`，未采集，未切 GZ `current`。2026-09-06 仓外 HTTPS 有界探测（`evidence/20260906-fanout-window-probe/`，无 facts 写入）对同一 `20260904` 窗 + 各一个已落库官方 seed：`dc_concept_cons` 60、`dc_member` 14、`fund_daily` 1，均 nonempty <10000 且非新整千边界，已写入 `active_evidence` 并 pinned `dc_concept` / `dc_index`；`fund_daily` 加入既有 `etf_basic` authority 且只加官方 `source_equals.list_status=L`。旧无界 3000/5000/2000 不再当作本切片 ingest 快照。未 restage `etf_mins`，未发明 `510300.SH`，未采 `etf_sz_cons`，未采集，未切 GZ `current`。2026-09-06 仓外 HTTPS 有界探测（`evidence/20260906-etf-fut-fanout-probe/`，无 facts 写入）对同一 `20260904` 窗 + 各一个已落库官方 seed：`etf_sz_cons` 50、`fut_holding` 2、`fut_wsr` 14，均 nonempty <10000 且非新整千边界，已写入 `active_evidence`；`etf_sz_cons` 加入既有 `etf_basic` authority 且只加官方 `source_equals.list_status=L`；`fut_holding` / `fut_wsr` 分别 pinned 到既有 `fut_basic` receipt 的 `symbol` / `fut_code` field authorities。旧无界 3000/2000/1000 不再当作本切片 ingest 快照。未 restage `etf_mins`，未发明 `510300.SH`，未采集，未切 GZ `current`。2026-09-06 仓外 HTTPS 有界探测（`evidence/20260906-member-fund-adj-probe/`，无 facts 写入）：`ci_index_member` / `index_member_all` 各用一个已落库 `security_master` `ts_code=000001.SZ` + 官方 `is_new=Y`（无 trade_date、无发明 l1/l2/l3）得到 nonempty 2 / 2；`fund_adj` 收窄为与 `fund_daily` 相同的 `etf_basic` `list_status=L` fanout + `trade_date=20260904`，一个已落库 seed `158003.SZ` 得到 nonempty 1。均 <10000 且非新整千边界，已写入 `active_evidence` 并 pinned 到既有 `security_master` / `etf_basic` authorities。旧无界 5000/2000/2000 不再当作本切片 ingest 快照。未 restage `etf_mins`，未发明 `510300.SH` / `AD`，未采集，未切 GZ `current`。`bse_mapping` / `fund_basic` / `sge_basic`
...
不得抬高硬预算或 unpause。`stock_company` 输入表表头是「必须」不是「必选」，且没有
...
上游稳定性不是接入、正常发布或对外提供只读数据服务的前置条件。工程验收验证我们能否按真实合同请求、保存、查询并准确说明结果；不要求我们先把上游改善到高于其实际服务水平。上游返回什么就如实处理什么，不把其 SLA 或文档描述自动当作我们的保证。

- 在已有授权、账户隔离、预算与数据完整性约束内，合同正确的接口可以接入、发布并供数。对外再分发权限仍按现有产品规则核对；本条不开放新公开入口或扩大权限。
- 已有可验证的数据按查询合同继续返回，并携带真实 `freshness`、`quality`、`degraded`、lineage、时间与原因。degraded 不等于一律停服；没有可验证数据时，返回既有合同规定的空结果或明确错误，不伪造补齐。失败的 cohort、无权限或损坏的 receipt 仍只阻断对应查询或范围。
- `stable` 只支持“连续稳定”的能力声明，不是开发、接入、正常发布、只读服务或既有预算内采集的统一开关。TA/Copilot 是否完成消费验收不决定 TD 能否独立供数。某个数据集失败不得封锁其它数据集或整个服务。
- TA 对样本时间、可交易性、收益评估和执行的要求由具体消费者自行判断，只影响对应样本、窗口或动作，不反向升级为 TD 接入或服务门禁。
- 不为供应商波动新增审批、连续成功天数、额外验收轮次、逐接口框架或新发布流程。复用现有测试、认证与发布检查；证据通过且无新改动或疑点即继续下一项。内部合同、存储、隔离或 API 缺陷仍由我们修复。

以下是采集与发布验收要求；其中外部质量状态不得被解释成上述工作的全局阻断：
...
运行代码或配置交付 merge 而未做 GZ cut = incomplete。纯文档变更按文档检查、PR/CI 与主线同步验收，不为刷新文档 SHA 重启数据服务；主线与实际 runtime SHA 分开记录，`STATUS.md` 不冒充运行验收。
...
每交易日 2–3 个仅为容量参考，不是每日限额；
历史、事件、参考数据的开发、合同复核、回填和发布可在非交易日继续，
实际采集按 cadence 与授权预算执行，只有依赖真实市场时段的回读需要等待该窗口。
检查节点也不是已就绪接口的最早发布日期。
...
及认证 query；在取得证据前保留当前 unobserved/degraded 等真实状态。回退使用上一
不可变 release，不回填哈希或修改历史数据。
