...
证明中的 `request_window={}` 可以是合法合同：对应 active provider binding 必须允许
空窗口。存在 `request_window_policy` 时仍通过同一窗口规范化校验；没有该 policy 时，
请求模板不得含需要窗口值的占位符。缺少必需日期、非法字段/格式或 provider 不匹配仍拒绝。
该规则只修正可选证明对合法空窗口的拒绝，不放宽逐行回执校验或上述单一采集序列约束。
