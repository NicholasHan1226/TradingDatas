...
| A-share fundamentals/reference | Tushare `fina_indicator`, financial statements, holders, pledge, company, concept, index membership, calendar, `fina_audit`, `fina_mainbz`, `bak_basic` | `/fundamentals`, `/reference`, `/industry`, `/tushare` | Daily/reference/reporting windows; `pledge_stat` and `pledge_detail` require `ts_code` and use bounded rotation, while `repurchase` uses one announcement-date window | Latest available reporting/reference snapshot | Active |
