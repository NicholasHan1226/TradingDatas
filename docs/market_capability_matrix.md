...
| A-share stock master | `cn.equity.security_master`; fixed `provider=tushare_stock_basic`, excluding `tushare_stock_company` | `/reference?table=stock_master&limit=500` plus signed `cursor` | One page per request; default/maximum 500; consumers must exhaust the snapshot-bound cursor chain | Preserve and validate each page's receipt state, freshness and lineage; one page is not universe-complete | Compatibility candidate; downstream migration and production deployment not asserted |
...
| Daily/reference/fundamentals | `/market_data`, `/reference?table=stock_master&limit=500&cursor=...`, `/industry`, `/fundamentals`, `/capital_flow`, `/tushare` | `/reference` is reserved for the cursor-paged A-share stock master; one response is not a complete universe. New consumers must use V1 catalog/query rather than add a provider-shaped API. |
