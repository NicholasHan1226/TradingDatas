...
| --- | --- | --- | --- | --- | --- |
| A-share intraday | Tushare `rt_min` | `/realtime_5min`, `/market_data?freq=5m`, `/tushare` | Trading window every 5 minutes; complete active universe in chunks of at most 300 symbols; retry only failed batches after request-level retries | Fresh-symbol coverage >= 80% during continuous trading; empty batch after bounded retries fails closed | Active |
