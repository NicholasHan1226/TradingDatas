...
Runtime proof is separate from configured capability. `logs/watchdog_inputs/interface_runtime.json` records the most recent attempt, success, rows read/written, provider failures, and SQLite status for every configured Tushare API. `/source_status` stays yellow for configured-but-unobserved interfaces and red for failed interfaces; a static `114/114 active` assignment alone is not runtime health proof.
