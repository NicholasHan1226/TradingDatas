...
- Only `stock_master` is active under `/reference`: use `/reference?table=stock_master&limit=6000` for SQLite `market_assets` rows filtered to `market=Ashare AND asset_type=stock`. The default limit is 6,000 and the hard maximum is 10,000. Responses retain provenance/freshness/quality/lineage; a missing database/table or no matching rows is degraded. There is no provider/CSV fallback, and every other legacy reference table remains degraded. This is the local code contract, not proof that the production runtime is deployed or populated.
...
| A-share daily/technical | Tushare `daily`, `stk_factor`, `stk_factor_pro`, `daily_basic`, `adj_factor` | `/market_data`, `/tushare` | `daily` is one market-wide trade-date snapshot after close; unique symbols must cover >=90% of the active universe. Unverified research interfaces remain single-symbol calls with a deterministic 300-symbol daily rotation, not comma batching | Latest trading day for `daily`; research dimensions complete one universe cycle in about 18 trading runs | Active, bounded by completeness/truncation guards |
...
| A-share stock master | Tushare asset-identity collectors persisted in `market_assets` | `/reference?table=stock_master&limit=6000` | Daily/reference window; API default 6,000, hard maximum 10,000 | Use row freshness and lineage; missing table or zero A-share stock rows is degraded | Canonical local contract; production deployment/data not asserted |
| A-share fundamentals/other reference dimensions | Tushare `fina_indicator`, financial statements, holders, pledge, company, concept, index membership, calendar, `fina_audit`, `fina_mainbz`, `bak_basic` | `/fundamentals`, `/industry`, `/tushare`; not another `/reference` table | Daily/reference/reporting windows; `pledge_stat` and `pledge_detail` require `ts_code` and use bounded rotation, while `repurchase` uses one announcement-date window | Latest available reporting/reference snapshot | Active read-model lanes; legacy CSV reference names remain degraded |
...
| US/HK | Tushare `us_daily`, `us_basic`, `hk_daily`, `hk_basic`, HK financials | `/market_data`, `/tushare`; `/reference` is not a US/HK fallback | Daily around local close windows | Latest effective trading day | Active but daily |
...
| ETF/fund/convertible bond/options | Tushare `etf_basic`, `fund_basic`, `fund_daily`, `fund_nav`, `fund_adj`, `fund_portfolio`, `fund_share`, `fund_div`, `cb_daily`, `cb_basic`, `cb_issue`, `opt_basic`, `opt_daily` | `/market_data`, `/tushare`; `/reference` is not a support-data fallback | Daily / reporting windows | Latest trading day or latest fund/NAV/reference/reporting date | Active as support data |
...
| Daily/reference/fundamentals | `/market_data`, `/reference?table=stock_master&limit=6000`, `/industry`, `/fundamentals`, `/capital_flow`, `/tushare` | `/reference` is reserved for the A-share stock master; reuse other read-model endpoints and filters before adding any provider-shaped API. |
...
Runtime proof is separate from configured capability. `logs/watchdog_inputs/interface_runtime.json` records the most recent attempt, success, rows read/written, provider failures, and SQLite status for every configured Tushare API. A zero-row provider response is explicit `empty`: it does not advance `last_success` and keeps `/source_status` yellow, including legacy ledger entries that previously called the same state success. Configured-but-unobserved interfaces also stay yellow and failed interfaces stay red; a static `114/114 active` assignment or a fresh shared table row from another provider is not runtime health proof.
