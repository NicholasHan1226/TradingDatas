...
- **A 股 realtime 批量读取修复**：2026-07-10 生产表确认 `20260710` 有 18,630 条 5 分钟行、覆盖 5,199 个代码，但旧 CTE 批量查询实测 23 秒并被 2.5 秒 SQLite 查询门禁 fail-closed。reader 改为先解析 `5min/5m` 最新交易日和最新 bar，再按精确 bar 快照读取；周/月线不再参与 realtime 最新日期选择，旧文件兜底保持退役。
