...
- **外部受控入口**：SharedSignals 正式外部域名规划为 `https://signals.tradingagent.cc`，目标 origin 为主服务器 `127.0.0.1:8082`。截至 2026-07-10，服务器 cloudflared 使用远程管理配置且现有 hostname 仅覆盖 `tradingagent.cc`、`api.tradingagent.cc`、`dashboard.tradingagent.cc`；`signals.tradingagent.cc` 尚未解析，需在 Cloudflare Tunnel/Access 侧新增 hostname/ingress 后才能对外使用。
