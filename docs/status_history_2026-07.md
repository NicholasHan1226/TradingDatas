...
- **外部受控入口**：SharedSignals 正式外部域名为 `https://signals.tradingagent.cc`。2026-07-10 已切换为 Cloudflare 橙云 A 记录指向主服务器 Nginx，再反代 `127.0.0.1:8082`；旧 Tunnel CNAME 已退役。当前 Cloudflare 模式为 `Full`，源站使用 2026-08-09 到期的临时自签名证书；一次未完成源站证书落盘的 `Full (Strict)` 切换触发 525 后已回滚并完成公网复测。Wangzhi/internal tier 账号、scope、并发和只读 API 已通过外网真实请求验证；外部 agent 仍必须使用 SharedSignals API key/JWT，不得绕过 API 直连 provider、CSV/NDJSON/SQLite 或旧目录。
...
- A股评分相关日频接口使用 90 天窗口并位于 P1 盘后日频；P0 `rt_min` 只处理当前 5 分钟快照，二者不混跑。
