# SharedSignals 2026-07 状态历史归档

> 本文件是历史归档，不是当前接手入口。当前事实以根层 [../STATUS.md](../STATUS.md)、`GET /health`、`GET /source_status` 和生产 live 输出为准。
>
...
- **资产主表污染修复**：发现 `stk_managers` 等非 identity 接口曾映射到 `market_assets`，把证券名称覆盖成高管姓名并制造 10 个虚假活跃股票分母。现将 `stock_company`、`fund_nav`、`share_float`、股东/高管/前十大/交易日历等改投 `market_factors`，将 `concept_detail`、`hs_const`、`index_weight` 改投 `market_relationships`，`index_basic` 改为真正的指数资产 identity；生产通过 `stock_basic` 重建 A股资产主数据后清理无法恢复的污染行。
...
- **API/module 扩展规则**：新增数据源必须先按 `config/api_module_catalog.yaml` 归类到模块、read-model 表和默认 HTTP surface；默认复用现有 22 个 API 路径，只有出现新查询形态、独立 SLA/auth scope、分页/限流模型或明确的新数据产品时才新增 endpoint。
...
- **DuckDB 权威行清理**：2026-07-10 全表核对识别出 12,500 条时间规范化前的分钟线旧主键行；`market_bars_daily`、`market_bars_intraday` 已纳入 SQLite 权威快照删除传播，镜像同步后必须逐表计数一致，否则任务失败并保持告警。
- **A 股 realtime 批量读取修复**：2026-07-10 生产表确认 `20260710` 有 18,630 条 5 分钟行、覆盖 5,199 个代码，但旧 CTE 批量查询实测 23 秒并被 2.5 秒 SQLite 查询门禁 fail-closed。reader 改为先解析 `5min/5m` 最新交易日和最新 bar，再按精确 bar 快照读取；内部批量上限从 5,000 对齐 API 的 10,000，避免当前 5,199 只 active universe 被静默截断；周/月线不再参与 realtime 最新日期选择，旧文件兜底保持退役。
- **公网探针 vantage 修复**：2026-07-10 DNS Tunnel 切换后，公网从外部工作站可正常返回 401，但广州直接访问 Cloudflare 出站不稳定；`external_api_probe` 改为经现有 SSH relay 由新加坡发起公网检查，同时仍把结果写回广州治理产物。
...
- **API 契约**：`/market_data` 已透传并支持 `freq`；`freq=daily` 读 `market_bars_daily`，`freq=1m/5m/15m/30m/60m` 读 `market_bars_intraday`，未传 start/end 时只取该标的最新 intraday 交易日；美股日线读取兼容 `AAPL` 与 `AAPL.US` 两种 symbol 写法并在 read model 中互查；A股国债逆回购 `repo_daily` 保留 `market_factors` 因子写入，并额外投影到 `market_bars_daily`，因此 `204001.SH` 可按日线收益率读取；`/capital_flow` 同时支持 `date` 和 `ts_code/start/end` 调用，返回 P1 盘后采集并直接入库到 `market_factors` 的 `moneyflow:*` 展开行；`/events` 只读取 SQLite `market_events`，不回退旧事件候选文件，并支持 `market`、`symbol/ts_code/subject_code`、`subject_type`、`event_type` 和 `limit` 过滤；`/fundamentals`、`/macro` 与 `/tushare` 同样必须把 `limit` 下推到 reader/SQL 层，外部应用不得依赖 API 外层裁剪来保护生产库；`/tushare?api_name=daily` 在未传代码和日期时会先限定 `market=Ashare` 与 `provider=tushare_daily`，定位最新交易日后只返回该日 rows，供 TradingAgent 等外部消费者做批量覆盖/流动性读取，避免在生产大表上无界排序；HTTP API 缺表/缺文件/无匹配或读侧超时时固定返回 `data: []` 并通过 `metadata.degraded` 与 `degraded_reasons` 暴露原因；`/pm_markets` 已优先返回带最新价的 Polymarket 市场并透出 `price/latest_price/latest_price_time`，`/pm_prices` 直接读取 `market_pm_prices` 价格快照供 PM edge、估值和研究回放使用；`/health` 使用读模型动态样例；`/sentiment` 与 `/realtime_5min` 支持 `limit` 输出限流，`/realtime_5min` 另支持 `market` 参数；`/capabilities` 有生成 registry + auth scope 兜底，能力 smoke 也改为 DB-first reader 样例；真实 `config/api_tokens.json` 已退出 Git 跟踪，仓库仅保留模板
- **A股 macro/sentiment/capital-flow evidence 闭环**：`/macro` 补齐 P4 宏观/利率/FX/全球指数 provider 列表，并兼容月度/季度 `event_time` 的日期范围过滤；`/capital_flow` 扩展为读取 `moneyflow`、`moneyflow_hsgt`、`margin`、`margin_detail` 四类 P1 盘后资金流因子；`/sentiment` 改为按 `reference/sentiment_event_types.yaml` 中配置的事件类型（默认 `sentiment`、`major_news`、`news`、`cctv_news`）从 `market_events` 派生读取，使事件 lane 的真实入库数据能通过 `/sentiment` 返回；已补充 `tests/test_ashare_evidence.py` 契约测试覆盖 reader 与 HTTP API。
- **API 安全**：JWT 默认禁用（需显式配置 `SHAREDSIGNALS_JWT_PUBLIC_KEY`+`SHAREDSIGNALS_JWT_ISSUER`）；Bearer token 与 `X-API-Key` 通过 `token_hash`/`sha256` 64 位摘要认证（设置 `SHAREDSIGNALS_TOKEN_SALT` 时由 `auth._hash_token()` 生成 PBKDF2-HMAC-SHA256 摘要）；scope-based 端点访问控制；外部隔离账号使用 `external_read` 可读完整数据面（含 `/tushare` read-model 输出）但不含 `/cache/invalidate` 运维控制；账号层级已拆为内部 `internal` 与未来 `starter/research/pro/enterprise` 分级套餐；`LOCALHOST_BYPASS` 默认关闭
- **外部受控入口**：SharedSignals 正式外部域名为 `https://signals.tradingagent.cc`。2026-07-10 先尝试 Cloudflare 橙云 A 记录直连广州 Nginx，但大陆未备案源站出现 ICP 拦截且源站 TLS 路径触发 525；该路径已退役。当前改为 Cloudflare Tunnel CNAME，由新加坡 cloudflared connector 通过 loopback-only SSH reverse tunnel 访问广州 `127.0.0.1:8082`，不公开 8082、不复制数据库到新加坡。Wangzhi/internal tier 账号、scope、并发和只读 API 保持不变；外部 agent 仍必须使用 SharedSignals API key/JWT，不得绕过 API 直连 provider、CSV/NDJSON/SQLite 或旧目录。
...
- **市场能力治理**：`docs/market_capability_matrix.md` 记录 Tushare 113 个生产配置接口与 114 个 `/tushare` 白名单接口的边界；`rt_fut_min` 为独立期货入口，当前 0 planned。
- **外部 agent 接入**：`GET /agent_config` 输出机器可读 API 配置、频率标签和禁止绕过规则；2026-07-10 起配置已列出完整 23 个 HTTP 路径（含 health/config/cache、`/source_status` 数据源治理状态、`/opening_gate`、业务数据、delegated projection 和 `/tushare`），不再只列 13 个主要端点；`docs/external_agent_api_prompt.md` 提供可复制 prompt。外部 agent 必须先读 `/health`/`/agent_config`/`/source_status`/`/opening_gate`，再通过业务端点或 `/tushare` 读取数据库输出。
- **数据源治理状态**：生产 `GET /source_status` 检查外部 API、市场频率标签、Tushare 114/114 active、API/module catalog、cron、health SLA 和 capability registry；A股盘中覆盖不足会通过 health SLA 使该状态不再假绿。
- **生产恢复与部署防复发**：2026-07-09 一次生产 deploy 在复制 27GB SQLite 快照时触发磁盘空间不足，旧 rollback 脚本错误使用不完整备份恢复，导致 `marketdata.sqlite` `quick_check` 报损坏；已暂停 cron/API 和写入进程，清理坏备份与损坏 sidecar，从 `/opt/investment/SharedSignals/data/marketdata.duckdb` 重建 SQLite 成功（13 表、4,388,166 行、`integrity_msg=ok`），恢复后 `/health=ok`、`/source_status=green`。`deploy.sh` 已增加可用空间门禁、临时快照、`quick_check` 校验和 `sharedsignals-api` 重启；`rollback.sh` 不再回退到“最近备份”，只接受同次 deploy 的精确备份，并在恢复前校验 SQLite、替换前清理 WAL/SHM。2026-07-10 又发现旧 deploy 的延迟回滚可在另一轮部署完成后覆盖新 HEAD，且旧回滚校验期间 P0/CNFutures cron 仍可写库、watchdog 可重启 API；deploy/rollback 现共用非阻塞发布锁和 read-model 独占维护锁，现役 cron 统一取共享维护锁并在维护时跳过。自动回滚同时校验 failed-deploy HEAD；发现当前 HEAD 已变化时拒绝恢复代码和数据库。SQLite 快照改用原生 backup API，恢复先写同目录临时文件、校验后原子替换，异常路径会尝试恢复 API 服务。
- **SQLite 巡检降载**：2026-07-10 生产权威库增长到约 6.9GB 后，30 分钟 patrol 的两次 `quick_check/integrity_check` 持续二十分钟以上并阻塞部署维护锁。现役 patrol/heal 改为浅层可打开/schema/轻量查询/WAL/锁验证；部署快照、备份源和恢复流程仍保留深度完整性检查，避免把灾难恢复强校验错误地放进高频巡检。
...
- **Tushare 全量能力计划**：`config/tushare_capability_plan.yaml` 覆盖 114 个 `/tushare` 白名单接口，按市场/模块分配 scheduled、independent、event_lane 或 planned 状态。
...
- **事件 lane 频率观察**：2026-07-09 17:40 CST 复核，事件 lane 最近两轮正常写入，17:00 用时 13.7 秒、17:30 用时 31.7 秒，最近 10 次平均约 43 秒且有 228.9 秒慢轮次；`/events` 最新 `collected_at=2026-07-09T09:30:33+00:00`，`health_sla` 仍为 ok。因此不提升整条事件 lane 频率，full event lane 继续保持 30 分钟；2026-07-09 起仅对 `news/major_news` 增加 15/45 分钟 supplemental pilot，公告、研报、CCTV 不加密。生产 crontab 中历史 `safe appended entries` 残留的重复 `tushare_events_collect` 与 `tushare_low_frequency_collect` 已删除，备份在 `logs/cron/marketgraph_crontab_before_remove_duplicate_events_20260709T094136Z.bak`。
- **新闻补充采集上线验证**：2026-07-09 17:51 CST 生产手动 smoke 仅运行 `news,major_news`，2 个接口合计 14.1 秒、写入 1900 行、`api_failures=0/2`、`sqlite_errors=0`；生产 crontab 已保留 full event lane 30 分钟，同时新增 `15,45 8-23 * * 1-6 SHAREDSIGNALS_EVENT_APIS=news,major_news` 补充任务且目标事件/低频行各只出现 1 次。调度变更前备份：`logs/cron/marketgraph_crontab_before_news_major_15m_20260709T095025Z.bak`。
...
- **planned 能力启用**：新增 `P7_low_frequency` 与 `cron/tushare_low_frequency_collect.sh`，把 A股/指数周月线改为每周独立低频采集；同时将 `suspend_d`、`namechange`、`ths_index`、`dc_index`、`index_classify`、`cb_basic`、`opt_basic`、`fund_share`、`fund_div`、`cb_issue`、`ft_limit` 纳入对应日频参考/事件/支持层；B1 关系/成分数据已新增 `market_relationships` 并启用 `ths_member`、`dc_member`、`index_member`、`index_member_all`；B2 日频支持数据启用 `ths_daily`、`dc_daily`、`opt_daily`、`fut_holding`；B3-B5 最后 8 个 planned 接口已启用为因子/披露/支持数据：`bak_basic`、`cyq_perf`、`cyq_chips`、`fina_audit`、`fina_mainbz`、`fund_adj`、`fund_portfolio`、`ths_hot`。能力计划当前为 109 scheduled + 5 event_lane + 1 independent + 0 planned。

## 二、已知问题

- libor/hibor 当前可为空返回；libor 属历史停更类接口，hibor 取决于当日源数据。`shibor_lpr` 已恢复：2026-07-04 生产 P4 回看 180 天采集 3 行，写入 `market_factors` 6 条。
- sync_daily.py 已从 CSV→SQLite 过渡桥收口为 provider rows 直接写 SQLite；后续只保留生产 crontab 日志、API 输出和 DuckDB 同步链路观察
...
- **历史记录（已退役，2026-07-08）**：P0 曾使用每轮 30 只的 `stk_mins`/`rt_min` 优先/轮转方案；2026-07-10 已由 `rt_min` 全市场分批覆盖替代。
- A股 P1 盘后任务曾把 12 个接口按 5,190 只 active universe 逐股扫描，2026-07-10 实测计划 62,398 calls，50 分钟只推进约 7,000 次。现将交易关键 `daily` 改为按 `trade_date` 全市场一次查询并要求唯一股票覆盖率 >=90%；`repurchase` 改公告窗口全市场增量；`pledge_stat/pledge_detail` 补 `ts_code`；其余未证明全市场/批量安全的研究接口保持单股调用但每日每接口确定性轮转 300 只，计划降到约 3,012 calls。结构扫描另发现 P2/P3 六个 `per_stock=true` 接口缺 `ts_code`，也已补参数并纳入 300 只轮转；测试门禁禁止任何逐股接口再次以空参数运行。返回行数撞 provider guard 或 coverage 不足均 critical。P0 `rt_min` 仍只处理当前 5 分钟快照，两者不混跑。
...
## 三、API 接口状态（HTTP 路径 22；业务/缓存端点 18；health/config 入口 4；health core 14 OK / 2 delegated skipped）

生产 HTTP API 暴露 22 个路径：4 个 health/config 入口（`/health`、`/capabilities`、`/agent_config`、`/source_status`）+ 18 个业务/缓存端点。`/source_status` 只读现有控制面产物，按 green/yellow/red 判断接口纳管、模块/API 目录、扩源 planned 队列、调度重复、SLA 和能力 registry；`/health` 当前只把 SharedSignals 自身供数能力计入核心健康，`events` 已由 SQLite `market_events` 原生承担，MarketGraph 研究图谱类端点 `associations`、`impacts` 仍标记为 delegated/skipped，不再通过软链依赖 MarketGraph 内部目录。HTTP surface、`/health` reader sample 和 capability smoke 三个数字口径不同，不应混用。
...
5. [x] **P0：直接写入 SQLite read model** — `sync_daily.py` 已从 provider rows 直接写入 SQLite，并同步进入 DuckDB mirror；CSV-only 成功路径已退役
...
20. [x] **历史方案（已退役）** — P0 rotating/priority hot pool 已于 2026-07-10 删除，现行方案是 `rt_min` 每批最多 300 只、每 5 分钟覆盖完整 active universe。
...
- [x] CNFutures 5 分钟采集删除 AKShare/Sina 备源；`rt_fut_min` provider 错误返回 `failed`，provider 正常 0 行返回 `empty`，不再用另一来源伪装为同一生产供数链。
- [x] 边界：期货 5 分钟生产供数必须来自受控 collector/read model；如需新增 CTP/券商或其它行情源，必须作为独立 collector 接入、入库、限流和健康检查，不做隐式 fallback。
...
- [x] `rt_fut_min` 已直接写入 `market_bars_intraday`，兼容 `code/time` 和 `ts_code/trade_time` 字段，写入 `market=Futures`、`provider=tushare_rt_fut_min`、`interval=5min`。
- [x] `market_bars_intraday` 新增可空 `bid_price/ask_price/bid_size/ask_size/last_trade_date/expiry_date`；`rt_fut_min` 入库层兼容 `bid1/ask1/bid1_volume/ask1_volume` 等字段，并可从 `market_assets` 回填合约到期字段；`fut_basic` 配置已补采 `last_ddate/delist_date` 作为回填源。
- [x] `/realtime_5min?market=Futures&ts_code=...` 已可读取 Futures 5 分钟 read model；reader 层会透传可空 L1 盘口字段，API 层仍只读、不生成交易判断。
- [x] `rt_fut_min` 采集已加强失败语义：provider 权限/接口/本地调用错误返回 `failed`；非空行情写入 SQLite 0 行也返回 `failed`，避免把高频交易支撑链路误判为正常空跑。AKShare/Sina 备源已退役。
...
- [x] 新增 `tools/collect_cn_futures_daily.py`：单日运行 `P6_other_daily/fut_daily`，支持 `--trade-date YYYYMMDD`、`--dry-run`，直接入库失败时透传非零退出码；禁用入库的旧开关已删除。
...
- [x] 补齐 read model 映射：`cn_gdp`、`sf_month`、`us_tycr`、`us_tbr`、`us_tltr`、`repo_daily` 写入 `market_factors`；`cctv_news`/`news` 自动生成 `event_hash` 写入 `market_events`；`index_global` 写入 `Global` 日线，`etf_basic`/`fut_basic` 写入资产表。
...
- [x] watchdog collector 日志扫描改为标签化失败匹配，只抓真实 `ERROR`/`FAILED`/`SQLITE_ERRORS`/`sqlite_failures>0`/`database is locked`，不再把 `sqlite_errors=0` 误判；生产 `--no-email` 巡检恢复 `collector_status=ok`。
- [x] 验证：`pytest -q` 通过（79 passed, 147 skipped）；`py_compile` 与 cron wrapper `bash -n` 通过。

### 2026-07-04 行业映射自动刷新、force reload 与默认日期修复

- [x] 历史：A股行业 CSV 生成任务已退役；当前 `/industry` 直接读取 SQLite `market_assets.sector`。
...
- [x] 生产验证：API PID 已强制重启至最新进程；`/health` OK，`/industry?ts_code=000001.SZ` 从 SQLite `market_assets` 读取且不降级，默认 realtime/sentiment 均不降级。
...
- [x] 已删除 `reference/` 和 `collectors/tushare/` 下旧 A 股兼容 wrapper 与 RSSCollector collector/config/feed/filter/bridge 软链，避免后续 agent 误把历史兼容入口恢复成现役采集链路。
- [x] `reader.get_realtime_5min()` 只读 `market_bars_intraday`，未传日期时自动使用该股票最新 intraday 日期；`reader.get_industry()` 只读 `market_assets`；`reader.get_sentiment()` 只读 `market_events(event_type='sentiment')`，不回退 CSV。
...
- [x] **历史记录（已退役）**：`stk_mins` 曾映射到 `market_bars_intraday`；2026-07-10 已从生产配置、白名单、映射和能力目录删除。
- [x] 验证：SharedSignals 全量测试 207 项通过；DuckDB wrapper 用生产 venv 实跑通过，`duckdb_merge.py` 状态 `ok`。
- [x] 历史观察项已关闭：A股 P0 改为 `rt_min` 全市场直接入库，不再补桥接 `stk_mins` CSV。
...
- [x] 历史桥接闭环完成：当日完成过 CSV→SQLite 过渡桥；当前已升级为 sync_daily provider rows 直接写入 SQLite read model。
...
### 2026-07-02 final deep audit CRITICAL/HIGH 清零修复（历史记录）
...
- [x] 历史 CSV 导入工具边界测试：空 CSV、BOM-only、全未知列、NULL byte、10 万行 chunk、特殊字符路径均通过；当前生产采集不以 CSV 导入作为成功路径
...
**已应用修复（Round 3，3 项）：**
7. [x] `tradingagent/shared/data/reader.py`：MarketGraphCSVReader 路径修复 — 仅作历史修复记录；当前目标是退役 TradingAgent 对 MarketGraph 内部 CSV 的直接读取。
...
- 历史断裂数据链已作为迁移背景保留；当前不再接受 CSV/NDJSON 作为生产兜底路径。
- API 服务器线程化改造
...
6. [x] 历史：旧 collector 通用调度器后续已退役，当前生产入口为 cron wrapper + direct-DB collector
...
13. [x] `reader.py`：历史 Tushare import 路径修复记录；当前 reader 已删除旧 A 股根路径常量。
...
| RSS | RSS collector（retired） | market_events / sentiment_signals | 已退役归档；恢复事件采集前必须按新的 SharedSignals collector 直接写 SQLite read model，不得恢复旧 RSSHub、旧文件 staging 或旧 bridge |
