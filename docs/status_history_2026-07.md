...
- **外部受控入口**：SharedSignals 正式外部域名规划为 `https://signals.tradingagent.cc`，目标 origin 为主服务器 `127.0.0.1:8082`。截至 2026-07-10，服务器 cloudflared 使用远程管理配置且现有 hostname 仅覆盖 `tradingagent.cc`、`api.tradingagent.cc`、`dashboard.tradingagent.cc`；`signals.tradingagent.cc` 尚未解析。SharedSignals 侧 Wangzhi/internal tier 账号、scope、并发和只读 API 已就绪并通过本机生产 API 验证；公网入口仍 blocked，因为当前可操作的本机 Cloudflare origin certificate 只能写 `vorra.cc` DNS zone，不能写 `tradingagent.cc`。需拿到 `tradingagent.cc` zone 的 DNS Edit + Tunnel Edit 权限，或在 Cloudflare 控制台对目标 tunnel 新增 `signals.tradingagent.cc -> http://127.0.0.1:8082` 后才能对外使用。
...
- **数据源治理状态**：2026-07-09 生产已启用 `GET /source_status` 与每日 08:05 `cron/source_governance_monitor.sh`，并输出 `logs/watchdog_inputs/source_governance_summary.txt` 中文摘要。`/source_status` 检查外部 API 22 个端点、市场频率标签、Tushare 115/115 active、API/module catalog 与 planned 扩源队列映射、关键 crontab 行、health_sla 和 capability registry。后续不需要人工追问“接口是否都用上/模块是否乱/扩源是否误启用”，只需读取 `/source_status` 和每日摘要中的 yellow/red 原因。
