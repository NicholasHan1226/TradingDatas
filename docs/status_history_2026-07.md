...
- **外部受控入口**：SharedSignals 正式外部域名为 `https://signals.tradingagent.cc`，目标 origin 为主服务器 `127.0.0.1:8082`。2026-07-10 已通过 Cloudflare Tunnel/DNS 暴露，Wangzhi/internal tier 账号、scope、并发和只读 API 已通过外网真实请求验证；外部 agent 仍必须使用 SharedSignals API key/JWT，不得绕过 API 直连 provider、CSV/NDJSON/SQLite 或旧目录。
