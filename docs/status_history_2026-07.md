...
- **API 安全**：JWT 默认禁用（需显式配置 `SHAREDSIGNALS_JWT_PUBLIC_KEY`+`SHAREDSIGNALS_JWT_ISSUER`）；Bearer token 与 `X-API-Key` 通过 `token_hash`/`sha256` 64 位摘要认证（设置 `SHAREDSIGNALS_TOKEN_SALT` 时由 `auth._hash_token()` 生成 PBKDF2-HMAC-SHA256 摘要）；scope-based 端点访问控制；外部隔离账号使用 `external_read` 可读完整数据面（含 `/tushare` read-model 输出）但不含 `/cache/invalidate` 运维控制；`LOCALHOST_BYPASS` 默认关闭
