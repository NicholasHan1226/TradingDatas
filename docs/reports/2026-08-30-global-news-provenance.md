...
Corrected-candidate independent review reran all 79 Firecrawl tests (7.56s), verified the frozen source/test hashes, and closed the credential/depth/node P1 with no remaining P0/P1. Parent-owned exact candidate CI remains required before merge; bounded collection and authenticated readback remain separate production gates. Production readback should use an existing `content_uid` equality filter derived from a fresh receipt-associated fact, request the new fields explicitly, compare parsed raw content and precision, and continue reporting unknown completeness. It must not add filters, recast receipt collection time as publication time, or label the candidate healthy simply to pass acceptance.
