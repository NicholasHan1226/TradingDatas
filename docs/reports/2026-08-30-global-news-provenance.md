...
Raw JSON is captured before overwritten reserved fields, so those original values remain available and subject to the existing sensitive-value scan. It is never truncated to satisfy a budget. Unknown provider keys continue to produce schema drift; oversized retained payloads fail the existing admission/resource budgets before a storage write. No increased response, row, batch, nesting or sensitive-scan budget is introduced.
