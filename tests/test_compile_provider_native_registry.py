...
def test_fresh_https_evidence_promotes_bounded_on_demand_local_event_window() -> None:
    ...
    major_news["cadence_class"] = "on_demand"
...
def test_wave4_exact8_active_evidence_is_formal_and_fail_closed() -> None:
    ...
    assert len(active) == 153
    assert len(bindings) - len(active) == 37
    assert wave4_exact8 <= active
    assert "forecast" in active
    assert "pledge_detail" in active
...
def test_preflight6_bse_fund_sge_active_evidence_is_formal_and_fail_closed() -> None:
    observations = _observations()
    active_evidence = observations["active_evidence"]
    assert isinstance(active_evidence, dict)
    cohort = ("bse_mapping", "fund_basic", "sge_basic")
    for api_name in cohort:
        assert active_evidence[api_name] == f"server-evidence/20260906-preflight6-{api_name}"
    assert "fund_company" not in active_evidence
    assert "stk_nineturn" not in active_evidence
    assert "stock_hsgt" not in active_evidence
    ...
    assert datasets["stk_nineturn"]["provider_bindings"][0]["activation_state"] == "paused"
    assert datasets["stock_hsgt"]["provider_bindings"][0]["activation_state"] == "paused"
    active = {
        api_name
        for api_name, dataset in datasets.items()
        if dataset["provider_bindings"][0]["activation_state"] == "active"
    }
    assert len(datasets) == 190
    assert len(active) == 153
    assert len(datasets) - len(active) == 37


def test_bak_daily_and_fund_manager_window_probe_active_evidence_is_formal() -> None:
    observations = _observations()
    active_evidence = observations["active_evidence"]
    assert isinstance(active_evidence, dict)
    assert (
        active_evidence.get("bak_daily")
        == "server-evidence/20260906-bak-fund-window-bak_daily"
    )
    assert (
        active_evidence.get("fund_manager")
        == "server-evidence/20260906-bak-fund-window-fund_manager"
    )
    assert active_evidence.get("fund_adj") == (
        "server-evidence/20260906-member-fund-adj-fund_adj"
    )
    assert "etf_mins" in active_evidence
    ...
    for api_name in ("bak_daily", "fund_manager"):
        binding = datasets[api_name]["provider_bindings"][0]
        assert binding["activation_state"] == "active"
        assert binding["entitlement_state"] == "active"
        assert datasets[api_name]["cadence_class"] == "on_demand"
        assert binding["ingest_contract_state"] == "ready"
        assert binding["max_rows_per_attempt"] == 10000
    fund_adj = datasets["fund_adj"]["provider_bindings"][0]
    assert fund_adj["activation_state"] == "active"
    assert fund_adj["ingest_contract_state"] == "ready"
    assert fund_adj["fanout"]["source_equals"] == {"list_status": "L"}
    assert datasets["etf_mins"]["provider_bindings"][0]["activation_state"] == "active"
...
def test_fanout_window_probe_active_evidence_is_formal() -> None:
    observations = _observations()
    active_evidence = observations["active_evidence"]
    assert isinstance(active_evidence, dict)
    assert (
        active_evidence.get("dc_concept_cons")
        == "server-evidence/20260906-fanout-window-dc_concept_cons"
    )
    assert (
        active_evidence.get("dc_member")
        == "server-evidence/20260906-fanout-window-dc_member"
    )
    assert (
        active_evidence.get("fund_daily")
        == "server-evidence/20260906-fanout-window-fund_daily"
    )
    assert (
        active_evidence.get("etf_sz_cons")
        == "server-evidence/20260906-etf-fut-fanout-etf_sz_cons"
    )
    assert (
        active_evidence.get("fut_holding")
        == "server-evidence/20260906-etf-fut-fanout-fut_holding"
    )
    assert (
        active_evidence.get("fut_wsr")
        == "server-evidence/20260906-etf-fut-fanout-fut_wsr"
    )
    assert active_evidence.get("fund_adj") == (
        "server-evidence/20260906-member-fund-adj-fund_adj"
    )
    ...
    assert datasets["fund_daily"]["provider_bindings"][0]["fanout"]["source_equals"] == {
        "list_status": "L"
    }
    assert datasets["etf_sz_cons"]["provider_bindings"][0]["fanout"]["source_equals"] == {
        "list_status": "L"
    }
    assert datasets["fut_holding"]["provider_bindings"][0]["fanout"]["source_field"] == (
        "symbol"
    )
    assert datasets["fut_wsr"]["provider_bindings"][0]["fanout"]["source_field"] == (
        "fut_code"
    )
    assert datasets["fund_adj"]["provider_bindings"][0]["activation_state"] == "active"
    assert datasets["etf_mins"]["provider_bindings"][0]["activation_state"] == "active"
    assert datasets["ft_mins"]["provider_bindings"][0]["activation_state"] == "paused"
    active = {
        api_name
...
def test_etf_mins_active_evidence_is_formal_and_ft_mins_stays_paused() -> None:
    observations = _observations()
    active_evidence = observations["active_evidence"]
    assert isinstance(active_evidence, dict)
    assert active_evidence.get("etf_mins") == "server-evidence/20260906-etf-ft-mins-etf_mins"
    assert "ft_mins" not in active_evidence
    registry = compile_provider_native_registry(
        _bundle(), observations_document=observations
    )
    ...
    etf = datasets["etf_mins"]["provider_bindings"][0]
    assert etf["activation_state"] == "active"
    assert etf["entitlement_state"] == "active"
    assert datasets["etf_mins"]["cadence_class"] == "on_demand"
    assert etf["resumable_fanout"] == {
        "cursor_contract_version": 2,
        "max_batches_per_run": 1,
    }
...
def test_wave7_tradedate_exact3_evidence_is_formal_and_fail_closed() -> None:
    ...
    # daily_basic left the tradeday ts_code fanout after multi-code queries
    # returned empty or were cut off.  It re-enters on a trade_date-only
    # snapshot, not this wave7 fanout evidence.  Empty ≠ success.
    tradeday = {"cyq_chips", "cyq_perf"}
    assert {api for api in tradeday if active_evidence.get(api) == ref} == tradeday
    assert active_evidence.get("daily_basic") != ref
    assert (
        active_evidence.get("daily_basic")
        == "server-evidence/20260825T-daily-basic-trade-date-only"
    )
    registry = compile_provider_native_registry(_bundle(), observations_document=observations)
    bindings = {d["provider_bindings"][0]["api_name"]: d["provider_bindings"][0] for d in registry["datasets"]}
    active = {api for api, binding in bindings.items() if binding["activation_state"] == "active"}
    ...
    assert len(active) == 153
    assert len(bindings) - len(active) == 37
    assert tradeday <= active
    assert bindings["daily_basic"]["activation_state"] == "active"
    assert bindings["daily_basic"]["request_template"] == {
        "trade_date": "${window.trade_date}"
    }
    assert bindings["daily_basic"]["fanout"] == {"strategy": "none"}
    assert "pledge_stat" in active
    assert "forecast" in active
    assert "pledge_detail" in active
    assert "stk_nineturn" not in active
...
def test_wave7_high_fanout_exact3_is_active_automatic_and_resumable() -> None:
    registry = compile_provider_native_registry(_bundle(), observations_document=_observations())
    datasets = {
        dataset["provider_bindings"][0]["api_name"]: dataset
        for dataset in registry["datasets"]
    }
    high_fanout = {"cyq_chips", "cyq_perf"}
    assert {api for api in high_fanout if datasets[api]["cadence_class"] == "postclose_daily"} == high_fanout
    assert {
        api
        for api in high_fanout
        if datasets[api]["provider_bindings"][0]["pagination"]["strategy"] == "none"
        and datasets[api]["provider_bindings"][0]["resumable_fanout"]
    ...
    daily_basic = datasets["daily_basic"]["provider_bindings"][0]
    assert daily_basic["activation_state"] == "active"
    assert daily_basic["probe_state"] == "executable"
    assert daily_basic["probe_block_reasons"] == []
    assert daily_basic["ingest_contract_state"] == "ready"
    assert daily_basic["request_template"] == {"trade_date": "${window.trade_date}"}
...
def test_cashflow_and_express_use_undated_ts_code_fanout() -> None:
    ...
    assert "cn.dataset.fund_adj" not in paused
    assert "cn.dataset.cashflow" not in paused
    assert "cn.dataset.express" not in paused


def test_forecast_and_fina_audit_use_success_capable_event_contracts() -> None:
    registry = compile_provider_native_registry(
        _bundle(), observations_document=_observations()
    )
    datasets = {
    ...
    forecast_binding = forecast["provider_bindings"][0]
    assert forecast["cadence_class"] == "event"
    ...
    assert fina_audit["cadence_class"] == "event"
    assert audit_binding["activation_state"] == "active"
    assert audit_binding["probe_state"] == "executable"
    assert audit_binding["ingest_contract_state"] == "ready"
    assert audit_binding["request_template"] == {}
    assert audit_binding["request_window_policy"] is None
    assert audit_binding["fanout"] == {
        "strategy": "dataset_field",
        "parameter": "ts_code",
        "source_dataset_id": "cn.equity.security_master",
        "source_field": "ts_code",
    ...
    assert audit_binding["resumable_fanout"] == {
        "cursor_contract_version": 2,
        "max_batches_per_run": 1,
    }
    paused = {
    ...
    assert "cn.dataset.fund_adj" not in paused
    assert "cn.dataset.pledge_detail" not in paused
    assert "cn.dataset.stk_nineturn" in paused
    assert "cn.dataset.cb_price_chg" in paused
    assert len(paused) == 37
...
def test_active_on_demand_fanouts_keep_resumable_batch_cap_and_activation() -> None:
    ...
    paused_capped = ("ft_mins",)
    for api_name in paused_capped:
        binding = bindings[api_name]
        assert binding["activation_state"] == "paused"
        assert binding["resumable_fanout"] == expected_progress
        live_binding = live.provider_binding(f"cn.dataset.{api_name}", "tushare")
        assert live_binding.activation_state == "paused"
        assert live_binding.resumable_fanout is not None
        assert live_binding.resumable_fanout.cursor_contract_version == 2
        assert live_binding.resumable_fanout.max_batches_per_run == 1
        assert live_binding.fanout is not None
        assert live_binding.fanout.batch_size == 1
    for paused in (
        "fund_nav",
        "fund_company",
        "stk_nineturn",
...
def test_wave7_pledge_stat_evidence_is_formal_and_exact16_resolved() -> None:
    observations = _observations()
    active_evidence = observations["active_evidence"]
    assert isinstance(active_evidence, dict)
    ref = "server-evidence/ashare-wave7-pledge-stat-single-20260812T1947CST"
    assert active_evidence.get("pledge_stat") == ref
    registry = compile_provider_native_registry(_bundle(), observations_document=observations)
    bindings = {d["provider_bindings"][0]["api_name"]: d["provider_bindings"][0] for d in registry["datasets"]}
    active = {api for api, binding in bindings.items() if binding["activation_state"] == "active"}
    assert len(bindings) == 190
    ...
    security_master_dependents = {
        "balancesheet", "cashflow", "cyq_chips", "cyq_perf", "express",
