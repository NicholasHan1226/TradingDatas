...
def _get_events_page_cached(
    ...
    time_expr = EVENT_CURSOR_TIME_SQL
    event_key_expr = EVENT_CURSOR_KEY_SQL
    revision_expr = EVENT_CURSOR_REVISION_SQL
    event_hash_expr = EVENT_CURSOR_HASH_SQL
    if cursor:
        cursor_time, cursor_event_id, cursor_revision, cursor_event_hash = (
            _decode_event_cursor(cursor)
        )
        where.append(
            f"({time_expr} < ? OR "
            f"({time_expr} = ? AND {event_key_expr} < ?) OR "
            f"({time_expr} = ? AND {event_key_expr} = ? AND {revision_expr} < ?) OR "
            f"({time_expr} = ? AND {event_key_expr} = ? AND {revision_expr} = ? "
            f"AND {event_hash_expr} < ?))"
        )
        params.extend(
            [
                cursor_time,
    ...
    rows, db_degraded = _sqlite_rows(
        ...
        f"{revision_expr} AS __cursor_revision, "
        f"{event_hash_expr} AS __cursor_event_hash FROM market_events "
        f"WHERE {' AND '.join(where)} "
        "ORDER BY __cursor_time DESC, __cursor_event_key DESC, "
        "__cursor_revision DESC, __cursor_event_hash DESC LIMIT ?",
        (*tuple(params), page_limit + 1),
        "market_events",
    )
    if db_degraded is None and rows:
        if any(not _valid_event_cursor_row(row) for row in rows):
            raise ValueError("event page contains row without stable event cursor")
        ...
        if has_more:
            ...
            sort_key = (
                ...
                last_row.get("__cursor_event_hash"),
            )
        ...
        for row in page_rows:
            ...
            row.pop("__cursor_event_hash", None)
        return _json_cached(
            lambda: {
...
def _valid_event_cursor_row(row: dict[str, Any]) -> bool:
    revision = row.get("__cursor_revision")
    return (
        isinstance(row.get("__cursor_time"), str)
        and bool(row.get("__cursor_time"))
        and isinstance(row.get("__cursor_event_key"), str)
        and bool(row.get("__cursor_event_key"))
        and isinstance(revision, int)
        and not isinstance(revision, bool)
        and isinstance(row.get("__cursor_event_hash"), str)
        and bool(row.get("__cursor_event_hash"))
    )


def _decode_event_cursor(cursor: str) -> tuple[str, str, int, str]:
...
def _build_data_plane_runtime() -> Any:
    """Private dependency hook for the one shared lazy runtime."""
...
def _legacy_query_rows(runtime: Any, invocation: Any) -> list[dict[str, Any]]:
    ...
    envelope = runtime.legacy.legacy_envelope(query_envelope)
    data = envelope.get("data")
    metadata = envelope.get("metadata")
    if type(data) is not list or type(metadata) is not dict:
        raise ValueError("legacy query envelope is invalid")
    degraded = bool(metadata.get("degraded"))
    freshness = dict(metadata.get("freshness") or {})
    quality = dict(metadata.get("quality") or {})
    reasons = list(metadata.get("degraded_reasons") or [])
    lineage = dict(metadata.get("lineage") or {})
    lineage.update(
        {
            "reader": "query_service_compat",
            "next_cursor": metadata.get("next_cursor"),
            "runtime": {
                "authority": lineage.get("authority"),
                "dataset_id": metadata.get("dataset_id"),
                "state": metadata.get("runtime_state"),
    ...
    source_id = str(envelope.get("source") or metadata.get("dataset_id") or "query_service")
    collected_at = metadata.get("observed_at")
    if not data:
        if not degraded:
            return []
        return [
...
def get_tushare(api_name: str, ts_code: str | None = None, start_date: str | None = None, end_date: str | None = None, **params: Any) -> list[dict[str, Any]]:
    """Translate one legacy Tushare read into the shared query service."""

    runtime = _build_data_plane_runtime()
    legacy_params = {"api_name": str(api_name)}
    for name, value in (
        ("ts_code", ts_code),
        ...
        if value is not None:
            legacy_params[name] = str(value)
    legacy_params.update(
        {name: str(value) for name, value in params.items() if value is not None}
    )
    invocation = runtime.legacy.tushare_request(legacy_params)
    return _legacy_query_rows(runtime, invocation)
...
def get_reference(
    ...
    table_key = raw_table.strip().lower()
    if normalize_stock_master_table(raw_table) is not None:
        runtime = _build_data_plane_runtime()
        legacy_params = {
            "table": raw_table,
            "limit": str(limit),
        ...
        if cursor is not None:
            legacy_params["cursor"] = str(cursor)
        invocation = runtime.legacy.stock_master_request(legacy_params)
        return _legacy_query_rows(runtime, invocation)

    bounded_limit = _bounded_limit(limit, 500, max_value=10000)
    lineage = {
