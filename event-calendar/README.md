...
1. `cn.dataset.forecast` 当前 activation_state=paused → 需恢复采集；
2. `cn.dataset.share_float` 当前 cadence_class=on_demand、快照停在 2026-07-31 → 需改为周期采集。

## 将来上线的路径

1. td-v1 两个事件数据集恢复采集并验证产出；
