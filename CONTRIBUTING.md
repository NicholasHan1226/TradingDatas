...
6. The pull-request CI is the routine Git merge gate. It runs four `pytest-shard` slices in parallel, each with `pytest-xdist -n auto`, and excludes only `slow` canary/timing suites. A failing, missing, cancelled, or stale CI result must not auto-merge.
