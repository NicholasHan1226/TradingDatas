...
8. Fork PRs and untrusted authors never auto-merge. A merge never deploys TD server/runtime. A `static/**` or `public-web/**` merge can dispatch `.github/workflows/deploy.yml` for that exact merge SHA. Both jobs always run: Pages readback is a single doctype check on `tradingdatas-admin.pages.dev`; public Worker readback polls `tradingdatas.com` until the checkout's exact hashed `/assets/*.js` is visible (12 attempts, 5s apart). A single curl after `wrangler deploy` is not that gate. GZ/immutable runtime is not auto-deployed.
