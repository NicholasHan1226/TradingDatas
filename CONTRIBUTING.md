...
4. Run relevant deterministic local validation before opening or updating the pull request. Data/runtime work must still obtain its provider/receipt/SQLite/catalog/query/consumer evidence where applicable. A correct contract that yields honest empty or vendor-side `provider_error` is an external blocker: log it and move on to the next connectable interface. Do not wait for perfect vendor data, invent extra release gates beyond the existing dual-auth catalog <15s deploy check, or treat that outcome as incomplete engineering. empty ≠ success; do not fake fills. See `docs/OPERATIONS.md`「Datas PM 接入口径」。
...
7. A successful CI run on a trusted same-repository PR is enough to merge when the author is `NicholasHan1226`, the PR is not a fork or draft, the base is `main`, and TradingDatas CI succeeded on the exact current head SHA. The author may `gh pr merge`; automerge uses the same conditions. The repository label `pm-merge` is optional acceleration or post-audit and must not block a green trusted PR. `controller-accepted`, `AUTODEV_RETURN_V1` comments, `decision=accepted`, `candidate=<sha>` comment parsing, and `change_class=M0` body declarations remain retired. Workflow-governance changes under `.github/workflows/` never auto-merge, so a PR cannot self-authorize its own gate; after CI, author `NicholasHan1226` merges those via GitHub without a Datas PM requirement.
8. Fork PRs and untrusted authors never auto-merge. A merge never deploys TD server/runtime. A `static/**` merge triggers only Cloudflare Pages, then performs a bounded readback of the published Pages route; GZ/immutable runtime is not auto-deployed. Daily acceptance is the GZ cut, not the GitHub merge: merge without GZ cut is incomplete.
9. Keep source/GitHub, release, effective runtime, provider receipt, API readback, and consumer evidence as separate states. Passing GitHub CI does not by itself prove production data health. Daily report/acceptance is GZ running SHA + dual catalog gate when applicable + proving receipts (non-empty SUCCESS when the vendor returned rows). `STATUS.md` / `main` tip alone is not acceptance.
...
Changes under `.github/workflows/` must not be self-authorizing via automerge. After CI they are merged by author `NicholasHan1226` via GitHub; Datas PM is not required. This does not create a routine human-review requirement. A normal application-code PR may not weaken, remove, or replace its own CI/automerge gate.
