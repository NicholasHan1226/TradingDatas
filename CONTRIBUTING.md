...
7. A successful CI run is evidence only. A narrow M0 PR may merge automatically only when its trusted same-repository author applies `automerge-m0`, its body declares `change_class=M0` and the exact current `candidate=<sha>`, and every changed path is limited to README, CONTRIBUTING, docs, tests or static. Every other PR is M1 and requires the Finance Delivery Controller's SHA-bound `AUTODEV_RETURN_V1` plus `controller-accepted`. Workflow-governance changes are always manually merged after Controller review.
8. Fork PRs and untrusted authors never auto-merge. A merge never deploys TD server/runtime. A `static/**` merge triggers only Cloudflare Pages, then performs a bounded readback of the published Pages route; server source, immutable release and runtime remain separate Controller gates.
