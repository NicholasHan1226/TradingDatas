"""Binance USDⓈ-M public metrics-dump provider adapter.

It deliberately implements only the public daily ``metrics`` zip download from
...
the SNI layer; the dump publishes one zip per symbol per UTC day whose rows
carry the same 5-minute open-interest facts as ``openInterestHist``.  There is
no account, testnet, order, or API-key surface, and funding rate is not
available from the dump at all.
"""
