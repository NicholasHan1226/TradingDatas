...
| 时间 | 动作 | 结果 |
|------|------|------|
| 2026-07-06 | 加强 CNFutures 5 分钟采集失败语义 | `rt_fut_min` 采集会保留 provider 错误码/权限错误并以 failed 退出；非空行情桥接 SQLite 写入 0 行时不再标记 ok；生产确认 Tushare `rt_fut_min` 权限不足后增加 AKShare/Sina 5 分钟模拟盘备源；API 白名单补入 `rt_fut_min` 便于只读接口自检 |
