...
- **数据源治理状态**：2026-07-09 生产已启用 `GET /source_status` 与每日 08:05 `cron/source_governance_monitor.sh`，并输出 `logs/watchdog_inputs/source_governance_summary.txt` 中文摘要。19:46 CST 生产验证 `/source_status=green`：外部 API 22 个端点、Tushare 115/115 个 allowlisted 接口均已纳入 active mode、planned backlog 为 0、关键 crontab 行无重复、health_sla 为 green、capability registry 为 13 OK / 0 degraded / 0 down / 4 skipped。后续不需要人工追问“接口是否都用上”，只需读取 `/source_status` 和每日摘要中的 yellow/red 原因。
...
生产 HTTP API 暴露 22 个路径：4 个 health/config 入口（`/health`、`/capabilities`、`/agent_config`、`/source_status`）+ 18 个业务/缓存端点。`/source_status` 只读现有控制面产物，按 green/yellow/red 判断接口纳管、调度重复、SLA 和能力 registry；`/health` 当前只把 SharedSignals 自身供数能力计入核心健康，`events` 已由 SQLite `market_events` 原生承担，MarketGraph 研究图谱类端点 `associations`、`impacts` 仍标记为 delegated/skipped，不再通过软链依赖 MarketGraph 内部目录。HTTP surface、`/health` reader sample 和 capability smoke 三个数字口径不同，不应混用。
