...
发布检查更新至 2026-09-06 02:20 Asia/Shanghai。源码、公开网站、数据运行面和真实商业开通分别验收；历史快照由 Git 保存。
...
- `7ef6bd19` 全新暂存进程权威对：境内 20.350 秒 / 192 项，Crypto 11.406 秒 / 240 项，监听约 43.5 秒（含覆盖索引全表 fault-in）。该 fault-in 伤害监听且不足以让首个 A 股 catalog <15s。剩余成本包括收据 notes I/O、双快照打开、registry ~0.73s 与序列化；本 follow-up 只落地可测的打开/预热 I/O 收缩，**15s 仍未证明**。
...
- 该 SHA 的全新暂存进程双认证 catalog 对未过既有 15 秒门：A 股 200 / **20.350s** / 192 datasets；Crypto 200 / 11.406s / 240 datasets；匿名 401 两侧成立。监听约 43.5s（含覆盖索引全表 fault-in）。本次 fault-in **没有**把 A 股首请求压到 15s 以下。**15s 仍未证明**。**GZ `current` 仍为 `a3106d68`**。没有声称采集。
- 本 follow-up 只改只读 I/O：监听前不再对覆盖索引做全表 `COUNT(*)`（这是 43.5s listen 的主要嫌疑，且未让首个 A 股 catalog <15s），改为同一最近收据窗口 + 逐数据集 `COUNT`/`MIN`/`MAX` 并丢弃结果；快照打开的收据可读性探测改为 `LIMIT 1`，与“不要在打开路径聚合整本收据账本”的既有注释一致。权威、认证、receipt 与 15s 门不变。本地测试可证明 SQL/步数合同，**不能**替代另一次暂存双认证对。empty ≠ success。
