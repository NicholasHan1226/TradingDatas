...
- `bse_mapping` / `sge_basic` 已在 `5644f631` 上取得非空 success receipt（248 / 13 行）。`fund_basic` 同批 `{}` 窗口 failed `resource_budget`（coverage 2884，认证 query 0 行）。未抬高 10000，未 restage，未 mass-unpause。`fund_company` 与 empty 项保持暂停。
