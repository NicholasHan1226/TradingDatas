...
- **行情采集**：稳定运行 — Tushare（P0-P6 分层接口）+ Binance（9 symbols，ticker 5min + klines）+ Polymarket（markets+prices，经新加坡 SSH relay 优先、本地 Mihomo/Clash 兜底，默认不直连兜底）→ SQLite + CSV/NDJSON 缓存；2026-07-07 起 P0 里服务 A股评分/候选池的 `daily`、`stk_factor`、`stk_factor_pro` 单独使用 90 天 lookback，并显式按 `ts_code/start_date/end_date` 逐股采集，保证 TradingAgent 技术面/估值类评分有足够历史；分钟线 `stk_mins/rt_min` 仍保持当日 5 分钟采集，不扩大为历史分钟回补，避免采集压力失控；CSV→SQLite bridge 已把 `stk_factor` 同时投到 `market_bars_daily` 与 `market_factors`，`daily_basic`/`stk_factor_pro` 投到 `market_factors`，避免估值/换手因子下载后落错日线表、不能被 TradingAgent 六维评分消费。
