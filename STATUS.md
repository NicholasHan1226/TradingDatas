...
- 本轮 observations 候选的 GitHub 基线为 `7ce02ea25e81a04cd08fbbc106c0b46a816d2e8d`，其历史包含通用 transport commit `7bf7f71`、数字字段 commit `c92b001` 与证据文档 commit `7ce02ea`：前者在一个共享 monotonic deadline 内完成 DNS snapshot direct-connect、TLS 1.3、仅 pre-send 节点切换、send 后零 replay及 `rate_limited` 诚实分类；数字字段修复一次扩展 provider output field grammar，并保留 API/参数/window 的严格 grammar。组合回归 619 项通过，Ruff/compile/diff-check 通过；该候选本身尚未提交或推送，服务器、production 与真实 HTTPS 全接口 readback 也尚未完成。
