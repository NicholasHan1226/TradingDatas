...
发布检查更新至 2026-09-06 12:15 Asia/Shanghai。源码、公开网站、数据运行面和真实商业开通分别验收；历史快照由 Git 保存。
...
- [PR #517](https://github.com/NicholasHan1226/TradingDatas/pull/517) 合入 `8fe3498fdf735d7bad45606496e168f9ef7ce55a`。精确主线 [34009342258](https://github.com/NicholasHan1226/TradingDatas/actions/runs/34009342258) 通过。本次无 `static/**` / `public-web/**`，未调度 Cloudflare。A 股与 Crypto 的 live `current` 现均为该 SHA。本 STATUS **不再 cut**。
...
- 读回 2026-09-06 12:13 Asia/Shanghai：两侧 `/opt/investment/releases/tradingdatas{,-crypto}/current` 均为 `8fe3498fdf735d7bad45606496e168f9ef7ce55a`（1090 文件，tree `5e55daa6…`，registry 逐字节 cmp 通过）。`tradingdatas-v1-internal.service` 与 `tradingdatas-crypto-v1-internal.service` active；`127.0.0.1:18082` / `18083` 在听。错误 unit `tradingdatas-api.service` inactive。匿名两侧 401。
...
- 公网管理服务此前已随 source 更新至 `d1140e914a11b1303173c9e05148d86421a788ac`。live 不可变采集/API `current` 现为 `8fe3498f`，不能再写成 `7986be6d`、`21a1fb63`、`5644f631` 或 `e8a96ccc`。较早公网目录的 `fut_daily`、`opt_basic` unobserved/0 行记录已被 `e8a96ccc` 上回执取代；`bse_mapping` / `sge_basic` 的旧 unobserved 记录已被 `5644f631` 上回执取代。`fund_basic` 的 E+O `resource_budget` failed 已被 `8fe3498f` 上 E-only success receipt 取代，但认证 query 仍是 0 行。`stk_nineturn` 仍 paused。公网页面本身不是本次 receipt 权威。
