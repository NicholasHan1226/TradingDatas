...
>
> 最后更新：2026-07-13（DuckDB source snapshot P0 已进入 GitHub main；production 仍为旧 runtime。18:47 live 根盘 92%、SQLite 15.80GB 且 P2 writer 仍运行，所有 snapshot/sync/migration/backup/deploy 动作紧急冻结）
...
- **2026-07-13 资源压力紧急门禁**：18:47 fresh read-only evidence 显示根盘 105,286,258,688 bytes 中仅余 8,273,600,512 bytes（92% used），authority SQLite 已增至 15,796,649,984 bytes；`P2_financial_daily` 仍运行且该 Python 进程累计 `write_bytes=34,906,140,672`。内存、swap、inode 和 OOM 不是当前瓶颈，主要风险是持续写入、IO pressure 与根盘耗尽。禁止启动 snapshot benchmark、DuckDB sync、migration、backup 或 deploy；不得 kill writer 或删除任何备份，除非 Nicholas 明确授权并先完成事务/回滚评估。精确保留清单见 [docs/resource_pressure_2026-07-13.md](docs/resource_pressure_2026-07-13.md)。
...
3. [ ] DuckDB snapshot P0 生产门禁：等待 P2 自然结束后先 fresh 回读 `df`、DB/WAL/SHM 最终大小、writer 退出码和备份归属；当前 92% used/8.27GB available 下禁止任何新写入型基准。之后仍需同时解决 code-only deploy migration gate、使 projected filesystem usage <=90%、保留 DuckDB temp 余量，并在获批的安全窗口证明 snapshot+sync+reconcile+cleanup <480 秒、16/16、identity mismatch=0、零残留、collector 无丢失和 `/source_status` 无 red。
