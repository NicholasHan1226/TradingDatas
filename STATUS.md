...
- 本地 `codex/cursor-finite-coverage` 已把官方 `index_basic` 市场表与 `stock_hsgt` 类型表写入文档快照（doc 94 / 398）。`stock_hsgt` 按官方 `HK_SZ`/`SZ_HK`/`HK_SH`/`SH_HK` 映射，ingest 原因已空，仍 paused，未激活。`fund_company` 输入段是「无，可提取全部」，没有 名称/类型/必选表，也没有输入后说明/枚举表，保持 `official_requiredness_unknown`。`stock_company` 输入表表头是「必须」不是「必选」，且没有输入后交易所说明表，保持 `official_requiredness_unknown` + `request_anchor_unresolved`。`index_basic` / `opt_daily` ingest 仍因 6000 / 15000 行完整性未决保持阻断。preflight ready 现为 5 且全部 paused。**15s 未证明**。**GZ current 仍为 `a3106d68`**。`fut_daily` / `opt_basic` 无生产采集。empty ≠ success。
