...
- **数据源治理状态**：2026-07-09 生产已启用 `GET /source_status` 与每日 08:05 `cron/source_governance_monitor.sh`。当前生产 `/source_status` 为 `yellow`：Tushare 115 个 allowlisted 接口均已分配 active mode、planned backlog 为 0、关键 crontab 行无重复、health_sla 为 green；唯一 yellow 来自 capability registry 的 5 个 degraded smoke 项，不代表数据源接口未纳管。`red_checks=0` 时不需要人工追问“接口是否都用上”，只需处理报告里的 yellow/red 原因。
