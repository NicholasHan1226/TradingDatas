...
- **事件 lane 频率观察**：2026-07-09 17:40 CST 复核，事件 lane 最近两轮正常写入，17:00 用时 13.7 秒、17:30 用时 31.7 秒，最近 10 次平均约 43 秒且有 228.9 秒慢轮次；`/events` 最新 `collected_at=2026-07-09T09:30:33+00:00`，`health_sla` 仍为 ok。因此当前不提升整条事件 lane 频率，继续保持 30 分钟；若未来加密，只能先对 `news/major_news` 做小流量试点，不能把公告、研报、CCTV 与整条 P6 子集一起加密。生产 crontab 中历史 `safe appended entries` 残留的重复 `tushare_events_collect` 与 `tushare_low_frequency_collect` 已删除，备份在 `logs/cron/marketgraph_crontab_before_remove_duplicate_events_20260709T094136Z.bak`。
