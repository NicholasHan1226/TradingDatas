...
> （被误判 drift 拒收）。改为要求 3 个核心键为子集、类型正确，额外键容忍但只映射
> 核心字段；drift 测试改测 fundingTime 类型错误。21 项测试全绿。

> **2026-08-16 OI 回填吞吐修正：** 让路协议按"每批必让"实现后实测仅 ~1.3 天/
> 小时。改为条件让路：仅当距下一个 5 分钟边界不足 60 秒才让路越界，其余时间
> 连续工作（测试冻结时钟覆盖让与不让两路）。安全语义不变。

> **2026-08-16 OI dump 解析兼容 provider 网格相位漂移：** 生产回填实证 2026-02
> 的 metrics zip 网格为 00:05–次日 00:00（8 月为 00:00–23:55），严格
> 00:00 起点校验把这类合法日判为 provider_error。解析改为接受任意完整、
> 连续、无重复、落在 [当日, 次日00:00] 内的 288 个 5 分钟网格点（行时间戳即
> 事实身份，不改写）。新增相位漂移回归测试（25 项全绿）。

> **2026-08-16 OI 回填性能修正：** 让路修复后回填仍零写入——根因是
> `_ingested_days` 在每个 (日, symbol) 上重推导（含 receipt 密码学校验），
> O(天数×数据集×receipt 表) 使进程 100% CPU 空转。改为回填启动时推导一次、
> 成功日落入内存集合（崩溃重跑仍从库重推导）。24 项测试全绿。
...
> 与"采集已发布且未入库的最新一日"的设计意图不符。已修正：provider_error 逐日
> 向前回看直至首个成功日；合同/校验漂移不回退、整轮 fail closed；仅最新日缺失且
> 未发布时数据集标 `pending_publication`、整轮 success；更早缺失日也失败判定为
> 真实中断、整轮诚实失败。新增两个回归测试。
