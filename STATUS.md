...
最后更新：2026-08-30 16:04 CST（消费者实读通过；续采发布正在生产预检）。
本文只保留当前可替换摘要；历史决策见
...
PR #396 已合入284b2f7且精确主线CI通过，但真实数据库预检发现生产优化加载路径遗漏fanout来源，以及日期变量覆盖时间参数两项P1，故未切换采集器。修复分支新增4项实际来源路径回归，组合188项通过，等待独立审查及新的精确CI。管理后台已自动更新到284b2f7，而18082及采集器仍为4ef9565，七项财务数据在管理后台暂显示合同/回执不一致；该差异不是数据删除或采集成功证明。后续以修正版通过真实预检、发布及认证读回消除此差异。分钟真实交易日及财务跨日生产证据仍待后续自然周期。
